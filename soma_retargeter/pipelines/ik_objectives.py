...
def _joint_target_residuals(
    joint_q: wp.array2d(dtype=wp.float32),      # (n_batch, n_coords)
    coord_targets: wp.array1d(dtype=wp.float32), # (n_coords)
    coord_masks: wp.array1d(dtype=wp.float32),   # (n_coords)
    weight: wp.array1d(dtype=wp.float32),        # (1)
    start_idx: int,
    # outputs
    ...
    residuals[problem, start_idx + coord_idx] = (
        joint_q[problem, coord_idx] - coord_targets[coord_idx]
...
def _joint_target_jac_analytic(
    ...
    problem, coord_idx = wp.tid()
    dof_idx = coord_to_dof[coord_idx]
    if dof_idx >= 0:
        jacobian[problem, start_idx + coord_idx, dof_idx] = weight[0] * coord_masks[coord_idx]
...
class IKJointTarget(ik.IKObjective):
    """Penalizes selected joint coordinates away from configured target values."""

    def __init__(self, coord_targets, coord_masks, weight=1.0):
        super().__init__()
        self.coord_targets_np = coord_targets.astype(np.float32)
        self.coord_masks_np = coord_masks.astype(np.float32)
    ...
    def init_buffers(self, model, jacobian_mode):
        ...
        self.coord_targets = wp.array(self.coord_targets_np, dtype=wp.float32, device=self.device)
        self.coord_masks = wp.array(self.coord_masks_np, dtype=wp.float32, device=self.device)
    ...
    def residual_dim(self):
        return len(self.coord_targets_np)
